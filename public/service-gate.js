(function () {
  ...
  async function check() {
    ...
    try {
      ...
      const refresh = () => { const en = getLang() === 'en'; const light = getTheme() === 'light'; overlay.dataset.theme = light ? 'light' : 'dark'; overlay.querySelector('h1').textContent = en ? (selected.title_en || 'Website temporarily unavailable') : (selected.title_zh || '网站暂不可用'); overlay.querySelector('p').textContent = en ? (selected.subtitle_en || 'Website temporarily unavailable') : (selected.subtitle_zh || '网站暂不可用'); themeBtn.textContent = light ? (en ? 'Dark mode' : '夜间') : (en ? 'Light mode' : '日间'); admin.textContent = en ? 'Administrator debug' : '管理员调试'; };
