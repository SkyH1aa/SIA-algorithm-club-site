(function(){
  ...
  const status=$('pk-status'),room=$('pk-room'),code=$('pk-room-code'),roomState=$('pk-room-state'),presence=$('pk-room-presence'),mineReady=$('pk-my-ready'),peerReady=$('pk-peer-ready'),message=$('pk-room-message'),arena=$('pk-arena'),hostHud=$('pk-self-hud'),peerHud=$('pk-peer-hud');
  ...
  async function createRoom(){try{if(state.room||state.scene)clearRoom();state.room=await rpc('platformer_pk_create_room',{});state.role='host';say('房间已创建，把邀请码发给对手。');await connect()}catch(error){say(error.message||'创建房间失败',true)}}
  async function join(){const invite=$('pk-code-input').value.trim().toUpperCase();if(!invite)return say('请输入邀请码',true);try{if(state.room||state.scene)clearRoom();state.room=await rpc('platformer_pk_join_room',{p_invite_code:invite});state.role='guest';say('已加入房间。');await connect()}catch(error){say(error.message||'加入房间失败',true)}}
  ...
  class PkArenaScene extends combat.Game{
    ...
    syncRival(){
      const remote=state.peerState;if(!remote||!this.rival?.active)return;const x=Phaser.Math.Clamp(Number(remote.x)||this.rival.x,this.rival.body.width/2,WORLD_W-this.rival.body.width/2);const remoteBottom=Number(remote.bodyBottom);const rawY=Number.isFinite(remoteBottom)?remoteBottom-this.rival.body.height/2:Number(remote.y);const y=Phaser.Math.Clamp(Number.isFinite(rawY)?rawY:this.rival.y,50,this.definition.worldH-this.rival.body.height/2);this.rival.setPosition(Phaser.Math.Linear(this.rival.x,x,.42),Phaser.Math.Linear(this.rival.y,y,.42));this.rival.body.updateFromGameObject();this.rival.hp=Math.max(0,Number(remote.hp??this.rival.hp));this.rival.maxHp=Math.max(1,Number(remote.maxHp??this.rival.maxHp));
      this.rival.shield=Math.max(0,Number(remote.shield??this.rival.shield??0));this.rival.maxShield=Math.max(0,Number(remote.shieldMax??this.rival.maxShield??0));this.rival.armor=Math.max(0,Number(remote.armor??this.rival.armor??0));this.rival.armorMax=Math.max(0,Number(remote.armorMax??this.rival.armorMax??0));this.rival.attack=Math.max(0,Number(remote.attack??this.rival.attack??0));this.rival.weaponLabel=this.weaponLabelFromRemote(remote);
      ...
      const left=remote.leftFace??(Math.cos(Number(remote.aim)||0)<0),moving=!!remote.moving,jumping=!!remote.jumping;this.rivalArt.setTexture(jumping?(left?'heroJumpL':'heroJumpR'):moving?(left?'heroMoveL':'heroMoveR'):(left?'heroStandL':'heroStandR')).setDisplaySize(52,78).setOrigin(.5,1).setPosition(this.rival.x,this.rival.body.bottom+1);
    ...
    weaponLabelFromRemote(remote={}){if(remote.mode==='melee')return remote.weaponArt==='weaponBlade'?'长刀':'匕首';const key=remote.weaponArt||'weaponPistol';return ({weaponPistol:'手枪',weaponShotgun:'霰弹枪',weaponSmg:'冲锋枪',weaponLaser:'激光枪',weaponSniper:'狙击枪'})[key]||'手枪';}
    getPkWeaponLabel(){if(this.combatMode==='melee')return this.meleeWeapon==='blade'?'长刀':'匕首';if(this.activeWeapon==='smg')return '冲锋枪';if(this.activeWeapon==='laser')return '激光枪';if(this.activeWeapon==='sniper')return '狙击枪';return this.spreadShots>0?'霰弹枪':'手枪';}
    getPkWeaponDamage(){if(this.combatMode==='melee')return Number(this.getMeleeWeaponStats?.().damage)||0;if(this.activeWeapon==='laser')return Number(this.laserDamage)||0;if(this.activeWeapon==='sniper')return Number(this.sniperDamage)||0;if(this.activeWeapon==='smg')return Number(this.smgDamage)||0;return Number(this.atk||0)+Number(this.ammoDamageBonus||0);}
    publishState(time){if(time-this.lastNetAt<NET_MS)return;this.lastNetAt=time;const p=this.player;const shotEvents=this.outgoingShots.splice(0,96);send('state',{seq:++state.stateSeq,x:p.x,y:p.y,bodyBottom:p.body.bottom,hp:this.hp,maxHp:this.maxHp,armor:this.armor,armorMax:this.armorMax,shield:this.energyShield,shieldMax:this.energyShieldMax,attack:this.getPkWeaponDamage(),energy:this.energy,aim:this.getWeaponMuzzle().angle,leftFace:this.leftFace,moving:Math.abs(p.body.velocity.x)>1,jumping:this.heroJumping,weaponArt:this.getWeaponTextureKey(),mode:this.combatMode,laserActive:this.activeWeapon==='laser'&&this.laserActiveUntil>time,hits:this.hitLog.slice(-24),shotEvents});}
    updateArenaHud(){const fmt=value=>this.formatHealth(Math.max(0,Number(value)||0));const rival=this.rival||{};const self=`我方　护盾 ${fmt(this.energyShield)}/${fmt(this.energyShieldMax)}　装甲 ${fmt(this.armor)}/${fmt(this.armorMax)}　生命 ${fmt(this.hp)}/${fmt(this.maxHp)}　攻击 ${fmt(this.getPkWeaponDamage())}　武器 ${this.getPkWeaponLabel()}`;const enemy=`敌方　护盾 ${fmt(rival.shield)}/${fmt(rival.maxShield)}　装甲 ${fmt(rival.armor)}/${fmt(rival.armorMax)}　生命 ${fmt(rival.hp)}/${fmt(rival.maxHp)}　攻击 ${fmt(rival.attack)}　武器 ${rival.weaponLabel||'手枪'}`;hostHud.textContent=self;peerHud.textContent=enemy;}
    damageEnemy(enemy,amount,source='ability',applyMaster=true){if(enemy!==this.rival||!enemy?.active||this.ended)return;const raw=Math.max(0,Number(amount)||0)*(applyMaster?this.masterDamageMultiplier():1)*(this.assassinationBuffUntil>this.time.now?this.assassinationDamageMultiplier:1)+this.armorOverloadDamageBonus;const knockback=(source==='melee'||source==='meleeUltimate')&&this.meleeWeapon==='blade'?40:0;const direction=knockback?(Math.sign(enemy.x-this.player.x)||Math.sign(Math.cos(this.getWeaponMuzzle().angle))||1):0;this.sendHit(raw,source,knockback?{knockback,direction}:{});this.meleeHitEffect?.(enemy,source==='melee'||source==='meleeUltimate'?0x9ef5ff:0xffd166);}
    ...
    sendHit(damage,source='ability',extra={}){if(Number.isFinite(damage)&&damage>0&&!this.ended){const hit={id:`${state.role}-${++this.hitSeq}`,damage,source,...extra,at:Date.now()};this.hitLog.push(hit);if(this.hitLog.length>48)this.hitLog.splice(0,this.hitLog.length-48);send('hit',hit);}}
    applyRemoteKnockback(distance,direction){const player=this.player,body=player?.body;if(!player?.active||!body||!distance)return;const dir=Math.sign(Number(direction)||0)||1;const half=body.width/2;let target=Phaser.Math.Clamp(player.x+dir*Number(distance),half,this.definition.w-half);const left=Math.min(player.x,target),right=Math.max(player.x,target);for(const tile of this.platforms.getChildren()){const b=tile.body;if(!b||b.top>body.bottom||b.bottom<body.top)continue;if(b.right>=left&&b.left<=right){target=dir>0?Math.min(target,b.left-half-1):Math.max(target,b.right+half+1);}}player.setPosition(Phaser.Math.Clamp(target,half,this.definition.w-half),player.y);body.updateFromGameObject();if(typeof player.setVelocityX==='function')player.setVelocityX(0);this.burstEffect(player.x,body.bottom-35,0x9ef5ff,.7);}
    receivePvpHit(packet){if(this.ended)return;const id=String(packet?.id||'');if(id&&this.receivedHitIds.has(id))return;if(id){this.receivedHitIds.add(id);if(this.receivedHitIds.size>600){const keep=[...this.receivedHitIds].slice(-300);this.receivedHitIds=new Set(keep);}}const amount=Math.max(0,Number(packet.damage)||0);if(!amount)return;if(Number(packet.knockback)>0)this.applyRemoteKnockback(packet.knockback,packet.direction);const source=String(packet.source||'');const attacker=source==='armorReflect'||source==='thornsHit'||source==='undying'?null:this.rival;combat.Game.prototype.damage.call(this,amount,attacker);if(this.hp<=0)this.finish(false);}
  ...
  function startArena(){if(!combat)return roomSay('单机战斗模块尚未加载，请刷新页面后重试。',true);disposeArena();arena.classList.remove('pk-hidden');state.finished=false;state.peerSeq=-1;state.stateSeq=0;state.peerState=state.peerState||{};const game=new Phaser.Game({type:Phaser.AUTO,parent:'pk-phaser',width:VIEW_W,height:VIEW_H,backgroundColor:'#091724',physics:{default:'arcade',arcade:{gravity:{y:900},debug:false}},scale:{mode:Phaser.Scale.FIT,autoCenter:Phaser.Scale.CENTER_BOTH},scene:PkArenaScene});state.scene=game.scene.getScene('PkArenaScene');window.setTimeout(()=>{const scene=state.scene;if(scene&&scene.game===game){try{scene.input.keyboard.enabled=true;scene.input.keyboard.start?.();game.canvas?.setAttribute('tabindex','0');game.canvas?.addEventListener('pointerdown',()=>game.canvas?.focus?.(),{passive:true});game.canvas?.focus?.();}catch(_){} }},0);}
