(function(){
  ...
  class PkArenaScene extends combat.Game{
    ...
    applyRemoteKnockback(distance,direction){const body=this.player?.body;if(!body||!distance)return;const dir=Math.sign(Number(direction)||0)||1;const half=body.width/2;let target=Phaser.Math.Clamp(this.player.x+dir*Number(distance),half,this.definition.w-half);const left=Math.min(this.player.x,target),right=Math.max(this.player.x,target);for(const tile of this.platforms.getChildren()){const b=tile.body;if(!b||b.top>body.bottom||b.bottom<body.top)continue;if(b.right>=left&&b.left<=right){target=dir>0?Math.min(target,b.left-half-1):Math.max(target,b.right+half+1);}}this.player.setPosition(Phaser.Math.Clamp(target,half,this.definition.w-half),this.player.y);this.player.body.updateFromGameObject();this.player.setVelocityX(0);this.burstEffect(this.player.x,this.player.body.bottom-35,0x9ef5ff,.7);}
