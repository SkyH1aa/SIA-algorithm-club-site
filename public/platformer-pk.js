(function(){
  ...
  const send=(event,payload)=>state.channel?.send({type:'broadcast',event,payload}).catch(()=>{});
  ...
  class PkArenaScene extends combat.Game{
    ...
    configurePvp(){
      this.pvp=true;this.pvpReady=false;this.lastNetAt=0;this.hitSeq=0;this.receivedHitIds=new Set();this.enemyMultiplier=1;this.bossEnemy=null;this.finishZone?.destroy();this.flag?.destroy();this.finishZone=null;this.flag=null;
    ...
    publishState(time){if(time-this.lastNetAt<NET_MS)return;this.lastNetAt=time;const p=this.player;send('state',{seq:++state.stateSeq,x:p.x,y:p.y,hp:this.hp,maxHp:this.maxHp,armor:this.armor,armorMax:this.armorMax,shield:this.energyShield,shieldMax:this.energyShieldMax,energy:this.energy,aim:this.getWeaponMuzzle().angle,leftFace:this.leftFace,moving:Math.abs(p.body.velocity.x)>1,jumping:this.heroJumping,weaponArt:this.getWeaponTextureKey(),mode:this.combatMode,laserActive:this.activeWeapon==='laser'&&this.laserActiveUntil>time});}
    ...
    sendHit(damage,source='ability'){if(Number.isFinite(damage)&&damage>0&&!this.ended)send('hit',{id:`${state.role}-${++this.hitSeq}`,damage,source,at:Date.now()});}
    receivePvpHit(packet){if(this.ended)return;const id=String(packet?.id||'');if(id&&this.receivedHitIds.has(id))return;if(id){this.receivedHitIds.add(id);if(this.receivedHitIds.size>600)this.receivedHitIds.clear();}const amount=Math.max(0,Number(packet.damage)||0);if(!amount)return;/* A network hit has no local enemy object. Passing rival here made armor reflection bounce back through Broadcast forever. */combat.Game.prototype.damage.call(this,amount,null);if(this.hp<=0)this.finish(false);}
    ...
    sendCreatedBullets(before){const added=(this.playerBullets?.getChildren()||[]).filter(b=>b.active&&b.body&&!before.has(b));if(added.length)send('shot',{bullets:added.map(b=>({x:b.x,y:b.y,vx:b.body.velocity.x,vy:b.body.velocity.y,angle:b.rotation,damage:Number(b.getData('damage'))||0,tint:b.tintTopLeft||0xfff4c2,expires:Math.max(250,Number(b.getData('expires'))-this.time.now)}))});}
    remoteShot(packet){if(this.ended||!this.enemyBullets||!this.player?.body||!Array.isArray(packet.bullets))return;packet.bullets.slice(0,180).forEach(spec=>{const x=Number(spec.x),y=Number(spec.y),vx=Number(spec.vx),vy=Number(spec.vy);if(!Number.isFinite(x)||!Number.isFinite(y)||!Number.isFinite(vx)||!Number.isFinite(vy))return;const bullet=this.enemyBullets.create(x,y,'bullet');bullet.body.allowGravity=false;bullet.setVelocity(vx,vy);bullet.setRotation(Number(spec.angle)||0);bullet.setTint(Number(spec.tint)||0xfff4c2);bullet.setData('damage',Math.max(0,Number(spec.damage)||0));bullet.setData('owner','peer');bullet.setData('expires',this.time.now+Math.min(2000,Math.max(250,Number(spec.expires)||1200)));});}
